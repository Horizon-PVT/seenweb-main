// Simplified admin ebook page - similar structure to blog
...
export default function AdminEbooks({ session }: any) {
    const [ebooks, setEbooks] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);
    const [showModal, setShowModal] = useState(false);
    const [editingItem, setEditingItem] = useState<any>(null);
    const [formData, setFormData] = useState({
        title: '', slug: '', coverImageUrl: '', pdfUrl: '', description: '', status: 'DRAFT', displayOrder: 0
    });
    ...
    const handleOpenModal = (item?: any) => {
        if (item) {
            setEditingItem(item);
            setFormData({ title: item.title, slug: item.slug, coverImageUrl: item.coverImageUrl || '', pdfUrl: item.pdfUrl, description: item.description || '', status: item.status, displayOrder: item.displayOrder || 0 });
        } else {
            setEditingItem(null);
            setFormData({ title: '', slug: '', coverImageUrl: '', pdfUrl: '', description: '', status: 'DRAFT', displayOrder: 0 });
        }
        setShowModal(true);
    };
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Xóa ebook này?')) return;
        try {
            await fetch('/api/admin/ebooks', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
        ...
        <AdminLayout session={session}>
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <h2 className="text-3xl font-bold text-white">Quản lý Ebook</h2>
                    <button onClick={() => handleOpenModal()} className="flex items-center space-x-2  bg-gradient-to-r from-[#008080] to-[#CDAD5A] text-white px-4 py-2 rounded-lg hover:opacity-90">
                        <Plus size={20} /><span>Thêm Ebook</span>
                    </button>
                </div>
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-700">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">PDF URL</th>
                        ...
                        <tbody className="divide-y divide-gray-700">
                            {loading ? <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Đang tải...</td></tr> :
                                ebooks.length === 0 ? <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Chưa có ebook</td></tr> :
                                    ebooks.map((item, idx) => (
                                        <tr key={item.id} className="hover:bg-gray-750">
                                            ...
                                            <td className="px-6 py-4 text-xs text-gray-400 truncate max-w-xs">{item.pdfUrl}</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-center space-x-2">
                                                    {item.status === 'PUBLISHED' && <a href={item.pdfUrl} target="_blank" rel="noopener noreferrer" className="p-2 text-blue-400 hover:bg-gray-700 rounded-lg"><Eye size={18} /></a>}
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
                    <div className="bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-700">
                        <div className="sticky top-0 bg-gray-800 border-b border-gray-700 px-6 py-4 flex items-center justify-between">
                            <h3 className="text-2xl font-bold text-white">{editingItem ? 'Chỉnh sửa Ebook' : 'Thêm Ebook mới'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div><label className="block text-sm font-medium text-gray-300 mb-2">Slug</label><input type="text" value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#CDAD5A]" /></div>
                            <div><label className="block text-sm font-medium text-gray-300 mb-2">Cover Image URL</label><input type="url" value={formData.coverImageUrl} onChange={(e) => setFormData({ ...formData, coverImageUrl: e.target.value })} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#CDAD5A]" /></div>
                            <div><label className="block text-sm font-medium text-gray-300 mb-2">PDF URL *</label><input type="url" required value={formData.pdfUrl} onChange={(e) => setFormData({ ...formData, pdfUrl: e.target.value })} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#CDAD5A]" /></div>
