...
export default function ScriptRefinerPage() {
    ...
    useEffect(() => {
        if (router.query.source === 'extension' && router.query.video) {
            ...
            if (isManual) {
                setOriginalScript(`👋 Signal from Extension!\n\nScript copied to Clipboard.\n\n👉 Press Ctrl+V to paste here.\n\nVideo: ${title}`);
            } else {
    ...
    return (
        <div className={`min-h-screen bg-gray-50 text-gray-900 font-sans flex flex-col transition-all duration-300 ${isFullScreen ? 'fixed inset-0 z-50' : ''} notranslate`} translate="no">
            <Head>
                <title>Script Refiner | Advanced AI Editor</title>
            </Head>
