...
interface StoryStudioToolProps {
  onBack?: () => void;
}

export default function StoryStudioTool({ onBack }: StoryStudioToolProps) {
  // --- STATE ---
  ...
  return (
    <div className="h-full bg-[#120c08] text-[#e3d5b8] font-sans selection:bg-[#d4af37] selection:text-black overflow-y-auto">
      {/* VINTAGE HEADER */}
      <header className="sticky top-0 left-0 right-0 h-16 bg-[#1a140f] border-b border-[#3e2c1e] flex items-center justify-between px-6 z-40 shadow-xl">
        <div className="flex items-center gap-4">
          {onBack && (
            <button onClick={onBack} className="text-[#8c7335] hover:text-[#d4af37] transition-colors flex items-center gap-2">
              <ArrowLeft size={18} />
            </button>
          )}
      ...
      <main className="pt-8 px-6 pb-20 max-w-6xl mx-auto">
        {/* WIZARD STEPS */}
